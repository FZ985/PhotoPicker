...
import com.cropview.anim.SimpleValueAnimator;
import com.cropview.anim.SimpleValueAnimatorListener;
import com.cropview.anim.ValueAnimatorV14;
import com.cropview.anim.ValueAnimatorV8;
import com.cropview.callback.crop.Callback;
import com.cropview.callback.crop.CropCallback;
import com.cropview.callback.crop.LoadCallback;
import com.cropview.callback.crop.SaveCallback;
...
public class CropImageView extends AppCompatImageView {
    ...
    private void handleStyleable(Context context, AttributeSet attrs,
        ...
        TypedArray ta = context.obtainStyledAttributes(attrs,
                R.styleable.scv_CropImageView, defStyle, 0);
        Drawable drawable;
        mCropMode = CropMode.SQUARE;
        try {
            drawable = ta
                    .getDrawable(R.styleable.scv_CropImageView_scv_img_src);
            if (drawable != null)
                setImageDrawable(drawable);
            for (CropMode mode : CropMode.values()) {
                if (ta.getInt(R.styleable.scv_CropImageView_scv_crop_mode, 3) == mode
                        .getId()) {
                    mCropMode = mode;
                    break;
                }
            ...
            mBackgroundColor = ta.getColor(
                    R.styleable.scv_CropImageView_scv_background_color,
                    TRANSPARENT);
            mOverlayColor = ta.getColor(
                    R.styleable.scv_CropImageView_scv_overlay_color,
                    TRANSLUCENT_BLACK);
            mFrameColor = ta.getColor(
                    R.styleable.scv_CropImageView_scv_frame_color, WHITE);
            mHandleColor = ta.getColor(
                    R.styleable.scv_CropImageView_scv_handle_color, WHITE);
            mGuideColor = ta.getColor(
                    R.styleable.scv_CropImageView_scv_guide_color,
                    TRANSLUCENT_WHITE);
            for (ShowMode mode : ShowMode.values()) {
                if (ta.getInt(
                        R.styleable.scv_CropImageView_scv_guide_show_mode, 1) == mode
                        .getId()) {
                    mGuideShowMode = mode;
                    break;
                }
            ...
            for (ShowMode mode : ShowMode.values()) {
                if (ta.getInt(
                        R.styleable.scv_CropImageView_scv_handle_show_mode, 1) == mode
                        .getId()) {
    ...
    private void drawDebugInfo(Canvas canvas) {
        ...
        int x = (int) (mImageRect.left + (float) mHandleSize * 0.5f
                * getDensity());
        int y = (int) (mImageRect.top + textHeight + (float) mHandleSize * 0.5f
                * getDensity());
        StringBuilder builder = new StringBuilder();
        builder.append("LOADED FROM: ").append(
                mSourceUri != null ? "Uri" : "Bitmap");
        canvas.drawText(builder.toString(), x, y, mPaintDebug);
        builder = new StringBuilder();

        if (mSourceUri == null) {
            builder.append("INPUT_IMAGE_SIZE: ").append((int) mImgWidth)
                    .append("x").append((int) mImgHeight);
            y += textHeight;
            canvas.drawText(builder.toString(), x, y, mPaintDebug);
            builder = new StringBuilder();
        } else {
            builder = new StringBuilder().append("INPUT_IMAGE_SIZE: ")
                    .append(mInputImageWidth).append("x")
                    .append(mInputImageHeight);
            y += textHeight;
            canvas.drawText(builder.toString(), x, y, mPaintDebug);
            builder = new StringBuilder();
        }
        builder.append("LOADED_IMAGE_SIZE: ").append(getBitmap().getWidth())
                .append("x").append(getBitmap().getHeight());
        y += textHeight;
        canvas.drawText(builder.toString(), x, y, mPaintDebug);
        builder = new StringBuilder();
        if (mOutputImageWidth > 0 && mOutputImageHeight > 0) {
            builder.append("OUTPUT_IMAGE_SIZE: ").append(mOutputImageWidth)
                    .append("x").append(mOutputImageHeight);
            y += textHeight;
            canvas.drawText(builder.toString(), x, y, mPaintDebug);
            builder = new StringBuilder().append("EXIF ROTATION: ").append(
                    mExifRotation);
            y += textHeight;
            canvas.drawText(builder.toString(), x, y, mPaintDebug);
            builder = new StringBuilder().append("CURRENT_ROTATION: ").append(
                    (int) mAngle);
            y += textHeight;
            canvas.drawText(builder.toString(), x, y, mPaintDebug);
        }
...
//				mPaintFrame);
        ...
        //左上
        canvas.drawLine(mFrameRect.left - mHandleSize / 2, mFrameRect.top - mHandleSize / 4, mFrameRect.left + mHandleSize * 2, mFrameRect.top - mHandleSize / 4, mPaintFrame);
        canvas.drawLine(mFrameRect.left - mHandleSize / 4, mFrameRect.top - mHandleSize / 4, mFrameRect.left - mHandleSize / 4, mFrameRect.top + mHandleSize * 2, mPaintFrame);
        //左下
        canvas.drawLine(mFrameRect.left - mHandleSize / 2, mFrameRect.bottom + mHandleSize / 4, mFrameRect.left + mHandleSize * 2, mFrameRect.bottom + mHandleSize / 4, mPaintFrame);
        canvas.drawLine(mFrameRect.left - mHandleSize / 4, mFrameRect.bottom + mHandleSize / 4, mFrameRect.left - mHandleSize / 4, mFrameRect.bottom - mHandleSize * 2, mPaintFrame);

        //右上
        canvas.drawLine(mFrameRect.right + mHandleSize / 2, mFrameRect.top - mHandleSize / 4, mFrameRect.right - mHandleSize * 2, mFrameRect.top - mHandleSize / 4, mPaintFrame);
        canvas.drawLine(mFrameRect.right + mHandleSize / 4, mFrameRect.top - mHandleSize / 4, mFrameRect.right + mHandleSize / 4, mFrameRect.top + mHandleSize * 2, mPaintFrame);
        //右下
        canvas.drawLine(mFrameRect.right + mHandleSize / 2, mFrameRect.bottom + mHandleSize / 4, mFrameRect.right - mHandleSize * 2, mFrameRect.bottom + mHandleSize / 4, mPaintFrame);
        canvas.drawLine(mFrameRect.right + mHandleSize / 4, mFrameRect.bottom + mHandleSize / 4, mFrameRect.right + mHandleSize / 4, mFrameRect.bottom - mHandleSize * 2, mPaintFrame);
...
//		Logger.log(TAG, "mScale="+mScale);
    ...
    private void setupLayout(int viewW, int viewH) {
        if (viewW == 0 || viewH == 0)
            return;
        setCenter(new PointF(getPaddingLeft() + viewW * 0.5f, getPaddingTop()
                + viewH * 0.5f));
        setScale(calcScale(viewW, viewH, mAngle));
        setMatrix();
        mImageRect = calcImageRect(new RectF(0f, 0f, mImgWidth, mImgHeight),
                mMatrix);
    ...
    private void moveHandleLT(float diffX, float diffY) {
        if (mCropMode == CropMode.FREE) {
            ...
            checkScaleBounds();
        } else if (mCropMode == CropMode.SQUARE_FIT_WIDTH) {
            mFrameRect.left = 0;
            mFrameRect.top += diffY;
            if (isHeightTooSmall()) {
                float offsetY = mMinFrameSize - getFrameH();
    ...
    private void moveHandleRT(float diffX, float diffY) {
        if (mCropMode == CropMode.FREE) {
            ...
            mFrameRect.right = mImageRect.right;
            mFrameRect.top += diffY;
            if (isHeightTooSmall()) {
                float offsetY = mMinFrameSize - getFrameH();
                mFrameRect.top -= offsetY;
            ...
            animator.addAnimatorListener(new SimpleValueAnimatorListener() {
                ...
                public void onAnimationUpdated(float scale) {
                    mFrameRect = new RectF(currentRect.left + diffL * scale,
                            currentRect.top + diffT * scale, currentRect.right
                            + diffR * scale, currentRect.bottom + diffB
                            * scale);
                    invalidate();
                }
    ...
    private Bitmap getRotatedBitmap(Bitmap bitmap) {
        ...
        rotateMatrix.setRotate(mAngle, bitmap.getWidth() / 2,
                bitmap.getHeight() / 2);
        return Bitmap.createBitmap(bitmap, 0, 0, bitmap.getWidth(),
                bitmap.getHeight(), rotateMatrix, true);
    }
    ...
    private Bitmap decodeRegion() {
        ...
        try {
            ...
            Rect cropRect = calcCropRect(originalImageWidth,
                    originalImageHeight);
            if (mAngle != 0) {
                ...
                cropRect = new Rect((int) rotated.left, (int) rotated.top,
                        (int) rotated.right, (int) rotated.bottom);
            }
            cropped = decoder.decodeRegion(cropRect,
            ...
            Log.e(TAG, "OOM Error: " + e.getMessage());
        } catch (Exception e) {
            Log.e(TAG, "An unexpected error has occurred: " + e.getMessage(), e);
        } finally {
            PickerUtils.closeQuietly(is);
        }
        return cropped;
    }
    ...
    private Rect calcCropRect(int originalImageWidth, int originalImageHeight) {
        ...
        int imageW = Math.round(getRotatedWidth(mAngle, originalImageWidth,
                originalImageHeight));
        int imageH = Math.round(getRotatedHeight(mAngle, originalImageWidth,
                originalImageHeight));
        return new Rect(Math.max(left, 0), Math.max(top, 0), Math.min(right,
                imageW), Math.min(bottom, imageH));
    }
    ...
    private Bitmap scaleBitmapIfNeeded(Bitmap cropped) {
        ...
        int outWidth = 0;
        int outHeight = 0;
        float imageRatio = getRatioX(mFrameRect.width())
                / getRatioY(mFrameRect.height());
        ...
        if (mOutputWidth > 0) {
            ...
            if (mOutputMaxWidth > 0 && mOutputMaxHeight > 0
                ...
                float maxRatio = (float) mOutputMaxWidth
                        / (float) mOutputMaxHeight;
                if (maxRatio >= imageRatio) {
                    outHeight = mOutputMaxHeight;
                    outWidth = Math
                            .round((float) mOutputMaxHeight * imageRatio);
                } else {
                    outWidth = mOutputMaxWidth;
                    outHeight = Math
                            .round((float) mOutputMaxWidth / imageRatio);
                }
            }
        }

        if (outWidth > 0 && outHeight > 0) {
            Bitmap scaled = PickerUtils.getScaledBitmap(cropped, outWidth, outHeight);
    ...
    private void saveToFile(Bitmap bitmap, final Uri uri) {
        ...
        try {
            ...
            Log.e(TAG, "An error occurred while saving the image: " + uri, e);
            postErrorOnMainThread(mSaveCallback);
        } finally {
            PickerUtils.closeQuietly(outputStream);
        }
    ...
    public void startLoad(Uri sourceUri, LoadCallback callback) {
        ...
        mExecutor.submit(new Runnable() {
            ...
            public void run() {
                mIsLoading = true;
                mExifRotation = PickerUtils.getExifOrientation(getContext(),
                        mSourceUri);
                int maxSize = PickerUtils.getMaxSize();
                int requestSize = Math.max(mViewWidth, mViewHeight);
                if (requestSize == 0)
                    requestSize = maxSize;
                try {
                    final Bitmap sampledBitmap = PickerUtils
                            .decodeSampledBitmapFromUri(getContext(),
                                    mSourceUri, requestSize);
                    mInputImageWidth = PickerUtils.sInputImageWidth;
                    mInputImageHeight = PickerUtils.sInputImageHeight;
                    mHandler.post(new Runnable() {
                        @Override
                        public void run() {
                            mAngle = mExifRotation;
            ...
            animator.addAnimatorListener(new SimpleValueAnimatorListener() {
                ...
                public void onAnimationFinished() {
                    ...
                    mHandler.post(new Runnable() {
                        ...
                        public void run() {
                            if (endListener != null) {
                                endListener.rotateEnd(getCroppedBitmap());
                            }
                        }
                    });
    ...
    public Bitmap getCroppedBitmap() {
        ...
        Bitmap cropped = Bitmap.createBitmap(rotated, cropRect.left,
                cropRect.top, cropRect.width(), cropRect.height(), null, false);
     ...
     */
    public void startCrop(Uri saveUri, CropCallback cropCallback,
                          SaveCallback saveCallback) {
        mSaveUri = saveUri;
        mCropCallback = cropCallback;
        mSaveCallback = saveCallback;
        Log.e(TAG, "crop##startCrop_mIsCropping:" + mIsCropping);
        if (mIsCropping) {
        ...
        mIsCropping = true;
        Log.e(TAG, "crop##startCrop_starting——start:");
        mExecutor.submit(new Runnable() {
            @Override
            public void run() {
                Log.e(TAG, "crop##startCrop_run:");
                Bitmap cropped;

                // Use thumbnail for crop
                if (mSourceUri == null) {
                    Log.e(TAG, "crop##startCrop_mSourceUri==null:");
                ...
                // Success
                if (cropped != null) {
                    Log.e(TAG, "crop##startCrop_cropped != null");
                    ...
                    mHandler.post(new Runnable() {
                        ...
                        public void run() {
                            if (mCropCallback != null)
                                Log.e(TAG, "crop##startCrop_成功回调");
                            mCropCallback.onSuccess(tmp);
                            mIsCropping = false;
                            if (mIsDebug)
                                invalidate();
                        }
    ...
    /**
     * Set interpolator of animation (Default interpolator is
    ...
    /**
     * Set fixed width for output (After cropping, the image is scaled to the
     * specified size.)
     *
    ...
    /**
     * Set fixed height for output (After cropping, the image is scaled to the
     * specified size.)
     *
    ...
    /**
     * Set maximum size for output (If cropped image size is larger than max
     * size, the image is scaled to the smaller size. If fixed output
     * width/height has already set, these parameters are ignored.)
     *
    ...
    public enum CropMode {
        FIT_IMAGE(0), RATIO_4_3(1), RATIO_3_4(2), SQUARE(3), RATIO_16_9(4), RATIO_9_16(
                5), FREE(6), CUSTOM(7), CIRCLE(8), CIRCLE_SQUARE(9), SQUARE_FIT_WIDTH(10);
    ...
    private RotateEndListener endListener;

    public void setRotateEndListener(RotateEndListener listener) {
        this.endListener = listener;
    }

    public interface RotateEndListener {
        void rotateEnd(Bitmap bitmap);
    }
}
